...
1. **Never read agent/subagent definition files yourself.** Your delegation primitive (e.g. Claude Code's `Agent` tool) auto-loads them for the subagent — reading them too in the orchestrator just doubles the cost for no benefit.
2. **Never inline large files into subagent prompts.** Tell the agent to read the file from disk itself instead. The subagent gets full content; your context stays lean.
...
Check the model's actual context window (not "it's Claude so 200K"). Some deployments are 1M, some are 200K. If you don't know, assume the smaller one — err toward leanness.
