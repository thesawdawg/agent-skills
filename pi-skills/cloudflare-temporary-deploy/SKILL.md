...
Every `wrangler deploy --temporary` call creates a **live, public URL** — even if it expires in 60 minutes unless claimed, it's real infrastructure other people could theoretically hit before then. Confirm with the user before the first deploy of a session (a quick "about to deploy this to a public workers.dev URL, OK?" is enough) — don't run it silently as a side effect of some other task.
...
- **No Cloudflare credentials present.** `--temporary` only works when Wrangler is unauthenticated: no OAuth login, no `CLOUDFLARE_API_TOKEN` / `CLOUDFLARE_API_KEY` env var, no `~/.wrangler` / `~/.config/.wrangler` cached OAuth. Don't set those vars for this session.
- Network egress to `cloudflare.com` and `workers.dev`.
- Using `--temporary` accepts Cloudflare's Terms of Service and Privacy Policy.

## How to Run

Use the `Bash` tool for every step. Always pin the version (`wrangler@latest` or `wrangler@4.102.0` or newer) so you don't accidentally run an old global wrangler that lacks the flag.

1. **Scaffold a minimal Worker** (skip if the project already exists). A Worker needs a `wrangler.toml` (or `wrangler.jsonc`) and an entry script. Minimal TypeScript example — write these with the `Write` tool:
