...
test_sync() {
  ...
  # A device that has never been asked about sync gets told, every brief —
  # until it is set up or the offer is explicitly declined.
  assert_contains "brief nudges while sync unconfigured" "$(dave brief)" "sync: not configured"
  jq 'del(.sync)' "$DAVE_HOME/config.json" > "$DAVE_HOME/c" && mv "$DAVE_HOME/c" "$DAVE_HOME/config.json"
  assert_contains "a sync-less config still nudges" "$(dave brief)" "sync: not configured"
  jq '.sync = {enabled:false, declined:true}' "$DAVE_HOME/config.json" > "$DAVE_HOME/c" && mv "$DAVE_HOME/c" "$DAVE_HOME/config.json"
  case "$(dave brief)" in
    *"sync: not configured"*) no "declined silences the nudge" "nudge still present" ;;
    *) ok "declined silences the nudge" ;;
  esac
  jq 'del(.sync.declined)' "$DAVE_HOME/config.json" > "$DAVE_HOME/c" && mv "$DAVE_HOME/c" "$DAVE_HOME/config.json"
