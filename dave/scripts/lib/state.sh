...
cmd_brief() {
  ...
  if sync_ready; then
    ...
    echo
  elif [ "$(config_bool '.sync.declined' false)" != "true" ]; then
    # The gap this line exists for: a device set up before sync (or a config
    # carried over without it) would otherwise never be asked. An explicit
    # "declined": true silences it — a decision, not a lapse.
    echo "=== SYNC ==="
    echo "sync: not configured on this device — run: dave.sh sync setup <git-remote-url>"
    echo
  fi
  echo "=== IDENTITY ==="
