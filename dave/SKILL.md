...
7. **Which remote holds the shared state?** → `sync.remote`. A private git repo
   (e.g. `dave-state`) is what makes the list the same list on every machine —
   without it, each device keeps its own truth and they drift. "None" is a real
   answer: write `"sync": {"enabled": false, "declined": true}` and he stops
   asking.
...
**Finish with sync — setup is not done until the state has a remote.** If one
was named, verify auth before touching `dave.sh`: `git ls-remote <url>` either
succeeds or it doesn't. On SSH, remember that sync calls plain `ssh` — a
non-default key name needs a `Host` entry in `~/.ssh/config`, or every pull and
push will fail while interactive tests happen to pass. Then:

```bash
scripts/dave.sh sync setup <remote-url>
```

An empty remote takes the local tree as its initial commit; a **non-empty remote
is adopted wholesale** — local state is replaced, only `config.json` survives.
If the local list might be newer than the remote's, say so and back up `~/.dave`
first. When it finishes, `sync status` should read clean; leave the device that
way.
...
`"sync": {"enabled": false, "declined": true}` marks a device that should stay
standalone — until then, `brief` prints the gap at every orient.
...
If `brief` reports `sync: not configured on this device`, this machine has never
been asked about shared state. Offer `sync setup <remote-url>` once — it is a
two-minute step, and an unsynced device is the one way the list lies to the
other machines. A no is a decision: record it as `"sync": {"enabled": false,
"declined": true}` in `config.json` and never raise it again.
